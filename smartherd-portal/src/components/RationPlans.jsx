...
export default function RationPlans() {
    ...
    const CSV_FIXED_COLS = new Set(['plan_id', 'phase', 'day_no', 'ramp_ingredient', 'forage_type', 'wt_min', 'wt_max', 'target_adg', 'est_cost_per_head_per_day']);
    ...
    const parseRationCSV = (text) => {
        ...
        lines.slice(1).forEach(line => {
            ...
            const row = {
                ...
                rampIngredient: (cellByHeader['ramp_ingredient'] || '').trim(),
    ...
    const handleExportV2Plan = (plan) => {
        ...
        const header = ['plan_id', 'phase', 'day_no', 'ramp_ingredient', 'forage_type', 'wt_min', 'wt_max', 'target_adg', ...ingredientNames, 'est_cost_per_head_per_day'];
                                    ...
                                    {distinctPenNames.map((penId, idx) => {
                                        ...
                                        return (
                                            <tr key={penId}>
                                                ...
                                                <td>
                                                    {(() => {
                                                        const planRampRows = penConfig.planId
                                                            ? rationRows.filter(r => r.planId === penConfig.planId && r.phase === 'INGREDIENT_RAMP')
                                                            : [];
                                                        if (planRampRows.length === 0) return null;
                                                        const rampIngredientName = planRampRows[0].rampIngredient;
                                                        return (
                                                            <>
                                                                <input
                                                                    type="date"
                                                                    class="form-control"
                                                                    title={`Start ${rampIngredientName} ramp — leave blank unless starting this pen's ramp`}
                                                                    style={{ minHeight: '32px', height: '32px', padding: '0.2rem 0.5rem' }}
                                                                    value={penConfig.rampStartDate || ''}
                                                                    onChange={e => handlePenFieldChange(penId, 'rampStartDate', e.target.value)}
                                                                    disabled={!isAdmin}
                                                                />
                                                                <label style={{ fontSize: '0.65rem', color: 'var(--text-muted)', display: 'block', marginTop: '0.15rem' }}>
                                                                    Start {rampIngredientName} ramp
                                                                </label>
                                                                {resolved?.rampInfo?.active && (
                                                                    <div style={{ fontSize: '0.68rem', color: 'var(--accent-gold)', marginTop: '0.2rem' }}>
                                                                        {resolved.rampInfo.ingredient} — day {resolved.rampInfo.day} of {resolved.rampInfo.total}
                                                                    </div>
                                                                )}
                                                            </>
                                                        );
        ...
        <div style={{ padding: '1rem 1.2rem' }}>
            <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
                ...
                <select class="form-control" style={{ width: 'auto', fontSize: '0.78rem' }} value={bracketFilterPhase} onChange={e => setBracketFilterPhase(e.target.value)}>
                    ...
                    <option value="INGREDIENT_RAMP">Ingredient Ramp</option>
